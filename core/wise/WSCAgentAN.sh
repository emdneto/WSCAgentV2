...
TEST_UCI=$(uci show wireless)

if [ "$?" = "1" ]; then
	error_exit "Missing wireless"
fi
...
then
  ...
  then
    ...
    then
      ...
      echo -e "The Wifi Slice Part with SSID was successfully created."
    else
    error_exit "The following Wifi Slice Part with SSID already exists: $ssid_name Skipping it's creation..."
    ...
    error_exit "There are no SSIDs available to be created."
    fi
...
then
  ...
  #echo -e "The Wifi Slice Part will be updated.\n"

  set_qos_bridge_int $ctrl_port_name $bw_rate $bw_burst

  save_changes
  echo -e "The Wifi Slice Part was successfully updated."
...
then
  ...
  then
    echo -e "The following Wifi Slice Part with SSID will be deleted: $ssid_name"
    ...
    echo -e "The Wifi Slice Part with SSID was successfully deleted."
  else
    #echo -e "#----Error----#\nThe following Wifi Slice Part with SSID doesn't exist: $ssid_name\nSkipping it's deletion...\n"
    error_exit "The following Wifi Slice Part with SSID doesn't exist: $ssid_name. Skipping it's deletion..."
  fi
else
